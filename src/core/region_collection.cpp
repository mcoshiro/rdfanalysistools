 ...
 */
void RegionCollection::set_flag_cuts(std::string name, int flag, std::string cuts) {
	//find region
 ...
 */
std::string RegionCollection::get_cuts(unsigned int region_idx, int flag) {
	if (flag == -1) {
		return region_cuts_default[region_idx];
	}
	//loop through flags to see if there are special cuts for this flag
	for (unsigned int flag_idx = 0; flag_idx < region_cuts_by_flag[region_idx].size(); flag_idx++) {
		if (flag == region_cuts_by_flag[region_idx][flag_idx].first) {
